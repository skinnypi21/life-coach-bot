  ...
  return (
    <button
      ...
      className={`w-9 h-9 rounded-lg border-2 text-sm font-bold transition-all ${
        selected ? selectedColors[value] : colors[value]
      }`}
    >
      {value}
    </button>
  ...
  async function fetchGoals() {
    try {
      ...
      if (data.success && data.goals) {
        setGoals(data.goals);
        // Pre-populate review with existing data
        ...
        setReview(r => ({ ...r, scores, goals: goalTexts, trackable, target, current }));
      }
    } catch {
      // If API not available, show empty form
              ...
              <div key={pillar.key} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div>
                  <label className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2 block">
                    Score this week {score > 0 && <span className="text-blue-600 font-bold">{score}/10</span>}
                  </label>
